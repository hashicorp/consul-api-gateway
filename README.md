...
mesh deployed on it. The installed version of Consul must be `v1.13.0` or greater.

The Consul Helm chart must be used, with specific settings, to install Consul on the Kubernetes
cluster. The Consul Helm chart must be version `0.44.0` or greater.  See the Consul API Gateway documentation for the required settings.
