...
func (c *ConnectCALeaf) Fetch(opts cache.FetchOptions, req cache.Request) (cache.FetchResult, error) {
	...
	expiresAt := minExpire.Add(utils.RandomStagger(maxExpire.Sub(minExpire)))
...
func (c *ConnectCALeaf) generateNewLeaf(req *ConnectCALeafRequest,
	...
	*/
	...
	// TODO: switch to gRPC Sign request
	if err := c.RPC.RPC("ConnectCA.Sign", &args, &reply); err != nil {
		// FIXME: scratch, cleanup before merging
		// gerr, ok := status.FromError(err)
		// if gerr.Code() == codes.ResourceExhausted {
		// if err.Error() == consul.ErrRateLimited.Error() {

		// TODO: restore the active root rotation logic
		/*
			// TODO: does this work or is string comparison needed?
	...
	// TODO: externalize this cache logic
	/*
		result.Value = &reply
...
func (r *ConnectCALeafRequest) Key() string {
	// TODO: restore EnterpriseMeta fields (namespace, partition, etc)
